...
def style_fig(fig, height=380, ytitle=None):
    fig.update_layout(height=height, margin=dict(
        l=20, r=20, t=60, b=40), font=dict(size=14, color='black'))
    if ytitle:
...
def load_and_prepare_data():
    ...
    if {"quantity", "price"}.issubset(df.columns):
        ...
        df["net_sales"] = df["gross_sales"] * \
            (1 - df.get("discount", 0).clip(0, 0.99))
...
with st.sidebar:
    ...
    if "order_date" in df.columns:
        ...
        start, end = st.date_input("Date range", value=(
            min_d.date(), max_d.date()), min_value=min_d.date(), max_value=max_d.date())
    ...
    for col in filter_cols:
        if col in df.columns:
            ...
            sel_filters[col] = st.multiselect(
                col.title(), options, default=options)

# Apply filters:
fdf = df.copy()
if start and end and "order_date" in fdf.columns:
    fdf = fdf[(fdf["order_date"].dt.date >= start)
              & (fdf["order_date"].dt.date <= end)]
...
c4.metric("Avg Order Value (AOV)",
          f"${total_net / orders:,.2f}" if orders else "$0.00")
...
def plot_sales_chart(df, col, chart_type="bar", y_col="net_sales", height=380):
    ...
    if col in df.columns:
        ...
        if chart_type in ["line", "area"]:
            ...
            agg = df.groupby(col, as_index=False)[
                y_col].sum().sort_values(y_col, ascending=False)
        ...
        st.plotly_chart(style_fig(fig, height=height, ytitle=y_col.replace(
            '_', ' ').title()), use_container_width=True)
    else:
...
if {"weekday", "category", "net_sales"}.issubset(fdf.columns):
    ...
    piv = fdf.pivot_table(index="weekday", columns="category",
                          values="net_sales", aggfunc="sum", fill_value=0)
    piv = piv.reindex(order, fill_value=0) 
    fig = px.imshow(piv, aspect="auto", color_continuous_scale="Blues",
                    labels=dict(color="Net Sales"), title=None)
...
if needed.issubset(fdf.columns):
    ...
    if not agg.empty:
        ...
        log_scale = st.toggle("Log scale (Y)", value=False,
                              help="Use for large sales differences.")
        ...
        fig.add_scatter(x=xs, y=a * xs + b, mode="lines",
        ...
        fig.update_layout(title=None, yaxis_title="Net Sales",
                          xaxis_tickformat=".0%", height=420)
        if log_scale:
            fig.update_yaxes(type="log", tickformat="~s")
        st.plotly_chart(fig, use_container_width=True)

        agg["sales_per_discount"] = agg["net_sales"] / \
            np.maximum(agg["avg_discount"], 1e-6)
        ...
        def format_func(x): return f"{x:,.0f}"

        with col_a:
            st.markdown("**Top Efficiency**")
            top3['sales_per_discount'] = top3['sales_per_discount'].apply(
                format_func)
            st.dataframe(top3[["category", "sales_per_discount"]].rename(columns={
                         'sales_per_discount': 'Sales/Discount'}), hide_index=True, use_container_width=True)
        with col_b:
            st.markdown("**Bottom Efficiency**")
            bottom3['sales_per_discount'] = bottom3['sales_per_discount'].apply(
                format_func)
            st.dataframe(bottom3[["category", "sales_per_discount"]].rename(columns={
                         'sales_per_discount': 'Sales/Discount'}), hide_index=True, use_container_width=True)
    else:
...
show_cols = [c for c in ["order_id", "order_date", "category", "region",
                         "quantity", "price", "discount", "gross_sales", "net_sales"] if c in fdf.columns]
st.dataframe(fdf[show_cols].sort_values(
    "order_date", na_position="last"), use_container_width=True, hide_index=True)
st.download_button("⬇️ Download filtered data (CSV)", data=fdf[show_cols].to_csv(
    index=False).encode("utf-8"), file_name="filtered_sales.csv", mime="text/csv")
